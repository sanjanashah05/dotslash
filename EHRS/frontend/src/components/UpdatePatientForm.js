...
const UpdatePatientForm = () => {
  ...
  useEffect(() => {
    const fetchPatientData = async () => {
      try {
        const response = await axios.get(`http://localhost:5000/api/patient/edit/${patientId}`);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.put(`http://localhost:5000/api/patient/edit/${patientId}`, formData);
      navigate('/dashboard'); 
    } catch (error) {
  ...
  return (
    <div className="p-6 sm:p-10 bg-black min-h-screen flex items-center justify-center">
      <div className="max-w-xl w-full bg-gray-950 shadow-lg rounded-lg p-6 sm:p-8">
        <h2 className="text-2xl font-semibold text-blue-300 mb-6">Update Patient</h2>
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            <label className="block text-sm font-medium text-blue-300 mb-1">Contact</label>
            <input
              type="text"
              name="contact"
